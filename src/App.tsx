...
const App = () => {
    ...
    const handleCalModChange = (newModId: string) => {
        ...
        const newWeaponNames = new Set(
            newMod?.weapons.filter((w) => w.caliber === effectiveCaliber).map((w) => w.name) ?? []
        );
        const newAmmoNames = new Set(
            newMod?.ammo.filter((a) => a.caliber === effectiveCaliber).map((a) => a.name) ?? []
        );
                        ...
                        <div className={`d-flex align-items-center gap-2 ${styles.caliberAmmoIcons}`}>
                            {calAmmoList.map((a) => {
                                ...
                                return ammoIcons[a.name] ? (
                                    ...
                                    >
                                        <img src={ammoIcons[a.name]} alt={a.name} className={styles.ammoIcon} />
                                    </div>
                                ) : (
                                    <span
                                        key={a.name}
                        ...
                        <div className="col-auto d-flex flex-wrap align-items-center gap-2">
                            {calWeaponList.map((w) => {
                                ...
                                return weaponIcons[w.name] ? (
                                    ...
                                    >
                                        <img src={weaponIcons[w.name]} alt={w.name} className={styles.weaponIcon} />
                                    </div>
                                ) : (
                                    <span
                                        key={w.name}
